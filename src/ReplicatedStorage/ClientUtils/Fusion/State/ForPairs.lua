...
function class:update(overwrite_keys: { [string]: boolean }?): boolean
	...
	for newInKey, newInValue in pairs(newInputTable) do
		...
		if overwrite_keys[newInKey] then
			shouldRecalculate = true
			didChange = true
		end
		if not shouldRecalculate and type(oldInputTable[newInKey]) == "table" then
			for key, entry in oldInputTable[newInKey] do
				if entry ~= newInValue[key] then
					shouldRecalculate = true
					break
				end
			end
