...
-- @treturn Binder
function Binder.new(tagName, constructor, params)
	local self = setmetatable({}, Binder)
	self.Core = _G.Core
	self._maid = self.Core.Utils.Maid.new()
	self._tagName = tagName or warn("Bad argument 'tagName', expected string")
	self._constructor = constructor or warn("Bad argument 'constructor', expected table or function")
	self._params = params
...
function Binder:GetClassAddedSignal()
	...
	self._classAddedSignal = self.Core.Utils.Signal.new() -- :fire(class, inst)
...
function Binder:GetClassRemovingSignal()
	...
	self._classRemovingSignal = self.Core.Utils.Signal.new() -- :fire(class, inst)
...
function Binder:_add(inst)
	...
	if type(self._constructor) == "function" then
		class = self._constructor(inst, self.Core, self._params)
	elseif self._constructor.Create then
		class = self._constructor:Create(inst, self.Core, self._params)
	else
		class = self._constructor.new(inst, self.Core, self._params)
	end
...
function Binder:_remove(inst)
	...
	if class.Destroy then
		...
		warn(
			("[Binder._remove] - Class %q no longer has destroy, something destroyed it!"):format(
				tostring(self._tagName)
			)
		)
	end
end
