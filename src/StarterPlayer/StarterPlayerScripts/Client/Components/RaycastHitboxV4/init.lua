...
function RaycastHitbox.new(object: any?, filter_obj: any?, custom_attachment_name: string?)
	local hitbox: any
	...
	if object and CollectionService:HasTag(object, DEFAULT_COLLECTION_TAG_NAME) then
		...
		if filter_obj then
			...
			raycastparams.FilterDescendantsInstances = { filter_obj }
			raycastparams.FilterType = Enum.RaycastFilterType.Exclude
		end
		hitbox = setmetatable({
			RaycastParams = raycastparams,
			DetectionMode = RaycastHitbox.DetectionMode.Default,
			CustomAttachmentName = custom_attachment_name,
			HitboxRaycastPoints = {},
