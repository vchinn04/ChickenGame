...
function Hitbox:Recalibrate()
	...
	local attachment_name = self.CustomAttachmentName
	if not attachment_name then
		attachment_name = DEFAULT_ATTACHMENT_INSTANCE
	end

	for _, attachment: any in ipairs(descendants) do
		if not attachment:IsA("Attachment") or attachment.Name ~= attachment_name then
			continue
		end
